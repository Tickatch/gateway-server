...
public class RedisQueueRepositoryImpl implements QueueRepository {
  ...
  private final RedisScript<List> removeAllowedTokenScript;
  private final RedisScript<List> cleanupExpiredTokensScript;
  ...
  public RedisQueueRepositoryImpl(
      ...
      RedisScript<List> removeAllowedTokenScript,
      RedisScript<List> cleanupExpiredTokensScript
  ...
  // 스케줄러가 주기적으로 호출할 메소드
  public Mono<RemoveExpiredUsersResult> cleanupExpiredTokens() {
  ...
  public Mono<RemoveAllowedUserResult> removeAllowedToken(String userId) {
    List<String> keys = Arrays.asList(ALLOWED_IN_HASH_KEY, WAITING_QUEUE_KEY);
